...
function getArquivoId(arquivo: ArquivoDocumento, origem: DocumentoOrigem): number {
  return origem === 'ENTIDADE'
    ? Number(arquivo.id_entidade_arquivo)
    : Number(arquivo.id_imovel_arquivo)
}

function tamanhoArquivo(bytes = 0): string {
...
function DocumentosPanel({ origem, ownerId }: DocumentosPanelProps) {
  const [tipos, setTipos] = useState<TipoDocumento[]>([])
  const [arquivos, setArquivos] = useState<ArquivoDocumento[]>([])
